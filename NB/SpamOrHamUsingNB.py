...
dataset = pd.read_csv('C:/Users/Mahmudur Limon/Desktop/Datathon/AllMoviesDetailsCleaned.csv' , sep = ';')


dataset=dataset.dropna()

#dataset['genres'] = dataset.Class.map({'cinema':0 , 'education':1})

dataset = dataset.drop(['original_title' , 'tagline' , 'id' , 'imdb_id' , 'production_companies' , 'production_countries' , 'release_date' , 'runtime' , 'spoken_languages' , 'status' , 'title'] , axis = 1)

labelencoder_X = LabelEncoder()
dataset['genres']= labelencoder_X.fit_transform(dataset['genres'])
